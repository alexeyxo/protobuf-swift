...
internal struct ProtobufUnittestImport {
  ...
  internal enum ImportEnum:Int32 {
    ...
    internal static func IsValidValue(value:Int32) ->Bool {
        if let check = ProtobufUnittestImport.ImportEnum(rawValue:value) {
            return true
        }
        return false
    }
  ...
  final internal class ImportMessage : GeneratedMessage, GeneratedMessageProtocol {
    ...
    internal class func parseFromData(data:NSData) -> ProtobufUnittestImport.ImportMessage {
      return ProtobufUnittestImport.ImportMessage.builder().mergeFromData(data, extensionRegistry:UnittestImportRoot.sharedInstance.extensionRegistry).build()
    }
