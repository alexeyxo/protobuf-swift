...
namespace google { namespace protobuf { namespace compiler { namespace swift {
    ...
    string FileName(const FileDescriptor* file) {
        string::size_type basename_start_pos = file->name().find_last_of('/') + 1;

        string basename = file->name().substr(basename_start_pos);

        return file->name().substr(0, basename_start_pos) + FilenameToCamelCase(StripProto(basename));
    }
